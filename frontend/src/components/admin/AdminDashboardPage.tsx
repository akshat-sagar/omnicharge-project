...
import { operatorService, planService, normalizeCollection } from '../../services/operatorPlanService';
import { rechargeService } from '../../services/rechargePaymentService';
import { useAppTheme } from '../../theme/AppThemeProvider';
import { Card, EmptyState, Spinner, StatCard, StatusBadge } from '../ui/index';
...
const CHART_COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#64748b'];
    ...
    <motion.div variants={stagger} initial="initial" animate="animate">
      <motion.div
        ...
        className={[
          ...
          isDark
            ? 'border-[#1f2b43] bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.18),transparent_30%),radial-gradient(circle_at_top_right,rgba(16,185,129,0.12),transparent_24%),linear-gradient(135deg,rgba(13,17,31,0.96),rgba(10,16,28,0.92))]'
            : 'border-white/70 bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.16),transparent_28%),radial-gradient(circle_at_top_right,rgba(16,185,129,0.1),transparent_20%),linear-gradient(135deg,rgba(255,255,255,0.92),rgba(239,244,255,0.86))] shadow-[0_24px_60px_rgba(15,23,42,0.08)]',
        ].join(' ')}
      >
        <div
              ...
              <div className="flex h-52 items-end gap-3">
                {trendData.map((item, index) => (
                  <div key={`${item.rechargeId}-${index}`} className="group flex flex-1 cursor-pointer flex-col items-center justify-end gap-2">
                    <div
                      className="w-full rounded-t-2xl bg-[linear-gradient(180deg,#60a5fa_0%,#2563eb_100%)] shadow-[0_10px_25px_rgba(37,99,235,0.24)] transition-all duration-200 group-hover:brightness-110 group-hover:shadow-[0_0_28px_rgba(59,130,246,0.65)]"
        ...
        <Card className={isDark ? 'bg-white/[0.03]' : 'bg-white/78'}>
          ...
          {loading ? (
            ...
            <div className="space-y-5">
              {[
                { label: 'Users', count: userCount, color: '#3b82f6' },

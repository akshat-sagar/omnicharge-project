    ...
    return (
      <div className="flex flex-col gap-1.5">
        {label && (
          <label
            htmlFor={inputId}
            className="auth-input-label text-sm font-medium text-surface-700"
          >
            {label}
        ...
        <div className="relative">
          ...
          <input
            ...
            type={actualType}
            className={[
              'auth-input-field w-full rounded-full border bg-white text-surface-900 backdrop-blur-xl',
          ...
          {isPassword && (
            <button
              ...
              className="auth-input-action absolute right-3 top-1/2 -translate-y-1/2 text-surface-400 transition-colors hover:text-surface-600"
            >
        ...
        {error && (
          <p className="auth-input-error flex items-center gap-1 text-xs text-red-600">
            <span className="material-icon text-[14px]">error</span>
            {error}
          </p>
        )}
        {!error && helperText && (
          <p className="auth-input-help text-xs text-surface-500">{helperText}</p>
        )}
      </div>
    );
  }

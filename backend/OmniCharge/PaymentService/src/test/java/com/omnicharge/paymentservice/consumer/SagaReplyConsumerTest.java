...
class SagaReplyConsumerTest {
    ...
    @BeforeEach
    void setUp() {
        consumer = new SagaReplyConsumer(transactionRepository, razorpayRefundService, rabbitTemplate);
    ...
    void onSagaDeadLetter_ShouldRefundSuccessfulTransaction() {
        ...
        when(razorpayRefundService.refundAndNotify(same(transaction), contains("rechargeId=5")))
                .thenReturn("rfnd_123");

        consumer.onSagaDeadLetter(event);

        verify(transactionRepository, org.mockito.Mockito.times(2)).save(transaction);
        verify(razorpayRefundService).refundAndNotify(same(transaction), contains("rechargeId=5"));
        org.junit.jupiter.api.Assertions.assertEquals(TransactionStatus.REFUNDED, transaction.getStatus());
        org.junit.jupiter.api.Assertions.assertEquals(
                "Recharge activation failed. Payment refunded.",
                transaction.getFailureReason()
        );
        verify(rabbitTemplate).convertAndSend(
                org.mockito.ArgumentMatchers.anyString(),
                org.mockito.ArgumentMatchers.eq("payment.failed"),
                org.mockito.ArgumentMatchers.any(PaymentSagaEvent.class)
        );
    }

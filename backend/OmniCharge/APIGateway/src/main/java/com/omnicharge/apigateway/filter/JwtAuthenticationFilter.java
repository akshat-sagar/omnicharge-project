...
import com.omnicharge.apigateway.exception.UnauthorizedException;
...
public class JwtAuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (email == null || email.isBlank() || role == null || role.isBlank() || userId == null) {
            log.error("JWT is missing required user claims for path: {}", path);
            return Mono.error(new UnauthorizedException("JWT is missing required user claims"));
        }

        String normalizedRole = role != null && role.startsWith("ROLE_") ? role : "ROLE_" + role;

        log.info("JWT claims extracted - Email: {}, Role: {}, UserId: {}, Path: {}", email, normalizedRole, userId, path);
        ...
        log.info("Request headers mutated with user information - Email: {}, Role: {}, Path: {}", email, normalizedRole, path);
